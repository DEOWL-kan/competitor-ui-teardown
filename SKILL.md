...
description: 逆向拆解竞品 App / 网页的某一屏是怎么做出来的，产出能直接照着做的设计规格。流程是「找候选 → 真机实测 → 经授权解包 APK → 量化分析 → 输出设计要点」。以下全都应该使用本技能：①点了名的——「分析一下 XX 的登录页」「Notion 的首页信息结构拆一下」；②没点名的——「同类产品的空状态都怎么做」「别人家的付费墙怎么设计」「找几个做得好的 onboarding 参考」；③只问一个机制细节的——「那个背景是视频还是静态图」「这个转场到底多长时间」「那个渐变怎么实现的」；④只有感受没有对象的——「我们的界面看起来很廉价，对手看着就贵，差在哪」「我们的不如它」。英文同理（analyse how X built their sign-in screen / why does their UI look more expensive / how long is that transition）。它的核心价值是把「看起来很高级」拆成可量化、可复现的机制（动效节奏、渐变结构、资源类型、色彩规格），⛔ 而不是停在主观描述。⛔ 不要用于给我方从零做设计、改我方文案配色、修我方页面 bug、竞品的定价与功能对比调研这几类——那些不是拆解实现机制。涉及下载安装包或提取竞品资源时会先征求授权。
---
...
- **`pm path` 返回多行是常态**（实测 10 个 App 里 9 个是 split APK）。
  **拿 `base.apk` 就够** —— 六个 App 实测，资源体积的 99%+ 都在 base 里，
